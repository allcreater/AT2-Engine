...
{
    auto texture = GlTexture::Make(m_renderer, Texture2D {TextureFormat::RGBA8Unorm, size}); //TODO: choose formats?
    texture->CopyFromFramebuffer(0, pos, size, {});
    return texture;
}

std::shared_ptr<ITexture> GlResourceFactory::CreateTexture(const Texture& declaration, bool renderTarget) const
{
    return GlTexture::Make(m_renderer, declaration);
}
