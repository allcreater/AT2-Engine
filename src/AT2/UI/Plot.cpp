...
{
    m_aabb.MinBound.x = startX;
    m_aabb.MaxBound.x = endX;
}
...
size_t Plot::EnumerateCurves(std::function<bool(std::string_view, const CurveData&, bool)> fn)
{
    ...
    std::for_each(m_curvesData.begin(), m_curvesData.end(), [&](decltype(m_curvesData)::value_type& x) {
        x.second.m_dataInvalidatedFlag &= !fn(
            x.first, x.second, x.second.m_dataInvalidatedFlag); //invalidation flag could be resetted, but not setted
        counter++;
    });
