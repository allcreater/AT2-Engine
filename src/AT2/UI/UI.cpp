...
{
    m_CanvasData.MeasuredSize.x =
        (HorizontalAlignment == Alignment::Stretch) ? possibleSize.x : std::min(possibleSize.x, m_Size.x);
    m_CanvasData.MeasuredSize.y =
        (VerticalAlignment == Alignment::Stretch) ? possibleSize.t : std::min(possibleSize.y, m_Size.y);

    switch (HorizontalAlignment)
    {
    case Alignment::Stretch: [[fallthrough]];
    case Alignment::Side1: m_CanvasData.Position.x = position.x; break;
    case Alignment::Side2: m_CanvasData.Position.x = position.x + (possibleSize.x - m_CanvasData.MeasuredSize.x); break;
    ...
    {
    case Alignment::Stretch: [[fallthrough]];
    case Alignment::Side1: m_CanvasData.Position.y = position.y; break;
    case Alignment::Side2: m_CanvasData.Position.y = position.y + (possibleSize.y - m_CanvasData.MeasuredSize.y); break;
    case Alignment::Center:
...
void Group::TraverseBreadthFirst(const std::function<void(const std::shared_ptr<Node>&)>& func)
{
    if (AT2::Utils::is_uninitialized(m_Parent))
        func(
            this->shared_from_this()); //yes, such a spike, but it seems there is no another way (except Node - ancestor from enable_shared_from_this() with another issues)
...
std::shared_ptr<StackPanel> StackPanel::Make(std::string_view name, Orientation orientation,
                                             std::initializer_list<std::shared_ptr<Node>> children,
                                             const glm::uvec2& size, Alignment vertical, Alignment horizontal)
{
...
std::shared_ptr<Group> Group::Make(std::string_view name, std::initializer_list<std::shared_ptr<Node>> children,
...
std::shared_ptr<Button> Button::Make(std::string_view name, const glm::uvec2& size, Alignment vertical,
