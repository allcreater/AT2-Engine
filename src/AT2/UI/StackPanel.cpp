...
void StackPanel::Measure(const glm::ivec2& position, const glm::uvec2& possibleSize)
{
    ...
    const auto elementCharacteristicalSize =
        (GetCanvasData().MeasuredSize - GetMinimalSize()) / std::max(numberOfAutoSizedChildren, 1u);
...
glm::uvec2 StackPanel::ComputeMinimalSize()
{
    ...
    Func horizontalFunc = [](glm::ivec2& accum, const glm::ivec2& size) {
        return glm::vec2(accum.x += size.x, std::max(accum.y, size.y));
    };
    Func verticalFunc = [](glm::ivec2& accum, const glm::ivec2& size) {
        return glm::vec2(std::max(accum.x, size.x), accum.y += size.y);
    };
    ...
    assert(minimalContentSize.x >= 0 &&
           minimalContentSize.y >= 0); //TODO: decide how to interpret negative values, or just forbid them :)
