...
namespace AT2
{
    ...
    public:
        GlDrawArraysPrimitive(GlDrawPrimitiveType mode, GLint first, GLsizei count) :
            GlDrawPrimitive(mode), m_firstIndex(first), m_count(count)
        {
        }

    public:
        void Draw() const override { glDrawArrays(static_cast<GLenum>(GetPrimitiveMode()), m_firstIndex, m_count); }
    ...
    public:
        GlDrawElementsPrimitive(GlDrawPrimitiveType mode, GLsizei count, IndicesType indicesType, const void* first) :
            GlDrawPrimitive(mode), m_count(count), m_indicesType(indicesType), m_firstIndex(first)
        {
        }
    ...
    public:
        ...
        {
            glDrawElements(static_cast<GLenum>(GetPrimitiveMode()), m_count, static_cast<GLenum>(m_indicesType),
                           m_firstIndex);
        }
    ...
    public:
        GlDrawElementsInstancedPrimitive(GlDrawPrimitiveType mode, GLsizei count, IndicesType indicesType,
                                         const void* first, GLsizei numInstances) :
            GlDrawPrimitive(mode),
            m_count(count), m_indicesType(indicesType), m_firstIndex(first), m_numInstances(numInstances)
        {
        }
    ...
    public:
        void Draw() const override
        {
            glDrawElementsInstanced(static_cast<GLenum>(GetPrimitiveMode()), m_count,
                                    static_cast<GLenum>(m_indicesType), m_firstIndex, m_numInstances);
        }
