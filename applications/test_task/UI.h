...
namespace AT2::UI
{
    ...
    {
        WindowRenderer(std::weak_ptr<Node> node, std::shared_ptr<WindowRendererSharedInfo> sharedInfo,
                       const glm::vec2& borderThickness = glm::vec2(3.0, 3.0),
                       const glm::vec4& color = glm::vec4(0.5, 0.5, 0.5, 0.3)) :
            m_Control(std::move(node)),
            m_SharedInfo(std::move(sharedInfo)), m_borderThickness(borderThickness), m_Color(color)
        {
        }
